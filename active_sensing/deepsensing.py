...
class DeepSensing(BaseEstimator, PredictorMixin):
      ...
      for i in tqdm(range(no)):     
        ...
        for t in range(seq_len):    
          ...
          for j in range(dim):          
            ...
            # See the prediction in comparison to original prediction    
            y_tilde[i, t, j] = \
            np.abs(self.pred_model.predict(temp_x)[0, seq_len - 1, 0] - y_hat[i, t, 0])
            # Restore the data
            temp_x[0, seq_len-1, j] = 0
  ...
  def fit (self, dataset):
    ...
    self.deep_sensing_class, self.deep_sensing_model = self.model_train(dataset, self.model_parameters, task = 'regression')
