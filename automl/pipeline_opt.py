...
from evaluation import BOMetric
import imputation
...
class PipelineOpt:
  def __init__(self, static_imputation_model_list, temporal_imputation_model_list, static_feature_selection_model_list,
               temporal_feature_selection_model_list, prediction_model_list, dataset_training, dataset_testing, task, metric_name, metric_parameters):
    ...
    static_imputation_list = [imputation.Imputation(imputation_model_name=x, data_type='static') for x in
                              static_imputation_model_list]
    temporal_imputation_list = [imputation.Imputation(imputation_model_name=x, data_type='temporal') for x in
    ...
    for x in static_feature_selection_model_list:
      ...
      static_feature_selection = \
        FeatureSelection(feature_selection_model_name=x[0],
                         feature_type='static', feature_number=x[1],
    ...
    for x in temporal_feature_selection_model_list:
      ...
      temporal_feature_selection = \
        FeatureSelection(feature_selection_model_name=x[0],
                         feature_type='temporal', feature_number=x[1],
                         task=task, metric_name=metric_name,
                         metric_parameters=metric_parameters)
    ...
    self.domain = [{'name': 'static_imputation', 'type': 'discrete', 'domain': list(range(len(static_imputation_list)))},
         {'name': 'temporal_imputation', 'type': 'discrete', 'domain': list(range(len(temporal_imputation_list)))},
         {'name': 'static_feature_selection', 'type': 'discrete', 'domain': list(range(len(static_feature_selection_list)))},
  ...
  def f(self, a):
    ...
    try:
      ...
      pipeline = PipelineComposer(static_imputation, temporal_imputation, static_feature_selection,
                                  temporal_feature_selection)
