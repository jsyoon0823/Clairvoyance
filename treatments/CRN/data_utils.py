...
def data_preprocess_counterfactuals(encoder_model, dataset, patient_id, timestep, treatment_options, fold, split,
  ...
  one_hot_treatments_encoder = np.concatenate([one_hot_treatments_encoder,
                                               np.zeros(shape=(
                                                 one_hot_treatments.shape[0], max_sequence_length - timestep,
                                                 one_hot_treatments.shape[-1]))], axis=1)
  ...
  dataset_crn_decoder['previous_treatments'] = one_hot_treatments[:, timestep - 1:timestep + projection_horizon - 1, :]
...
def process_seq_data(dataset, states, projection_horizon):
  ...
  for i in range(num_patients):
    ...
    for t in range(1, sequence_length):  # shift outputs back by 1
      ...
      seq2seq_current_covariates[total_seq2seq_rows, :max_projection, :] = outputs[i, t - 1:t + max_projection - 1, :]
