...
class CRN_Base():
  ...
  def train(self, dataset_train, dataset_val):
    ...
    with tf.variable_scope(self.name_scope, reuse=tf.AUTO_REUSE):
      ...
      for epoch in range(self.num_epochs):
        ...
        alpha_current = (2. / (1. + np.exp(-10. * p)) - 1) * self.max_alpha
        ...
        for (batch_current_covariates, batch_previous_treatments, batch_current_treatments, batch_init_state,
          ...
          feed_dict = self.build_feed_dictionary(batch_current_covariates=batch_current_covariates,
                                                 ...
                                                 batch_outputs=batch_outputs, alpha_current=alpha_current)
        ...
        logging.info(
          ...
          "treatment loss = {} | current alpha = {} ".format(epoch, training_loss, training_loss_outcomes,
  ...
  def build_feed_dictionary(self, batch_current_covariates, batch_previous_treatments,
                            batch_current_treatments, batch_init_state, batch_active_entries,
                            batch_outputs=None,
                            alpha_current=1.0, training_mode=True):
  ...
  def get_balancing_reps(self, dataset):
    ...
    for (batch_current_covariates, batch_previous_treatments,
         batch_current_treatments, batch_init_state, batch_active_entries) in self.gen_epoch(dataset,
      ...
      feed_dict = self.build_feed_dictionary(batch_current_covariates=batch_current_covariates,
                                             ...
                                             batch_current_treatments=batch_current_treatments,
                                             batch_active_entries=batch_active_entries,
                                             batch_init_state=batch_init_state, training_mode=False)
  ...
  def get_predictions(self, dataset, b_with_uncertainty=False):
    ...
    for (batch_current_covariates, batch_previous_treatments,
      ...
      feed_dict = self.build_feed_dictionary(batch_current_covariates=batch_current_covariates,
                                             ...
                                             batch_current_treatments=batch_current_treatments,
                                             batch_init_state=batch_init_state,
                                             batch_active_entries=batch_active_entries, training_mode=False)
  ...
  def compute_loss_treatments(self, target_treatments, treatment_predictions, active_entries):
    ...
    cross_entropy_loss = tf.reduce_sum(
      (- (target_treatments * tf.log(treatment_predictions + 1e-8) + (1 - target_treatments) * tf.log(
        1 - treatment_predictions + 1e-8))) * active_entries) \
  ...
  def compute_loss_predictions(self, outputs, predictions, active_entries):
    ...
    if self.task == 'regression':
      ...
      loss = tf.reduce_sum(
        (- (outputs * tf.log(predictions + 1e-8) + (1 - outputs) * tf.log(1 - predictions + 1e-8))) * active_entries) \
             / tf.reduce_sum(active_entries)
    else:
  ...
  def compute_validation_loss(self, validation_dataset):
    ...
    for (batch_current_covariates, batch_previous_treatments, batch_current_treatments, batch_init_state,
         batch_outputs, batch_active_entries) in self.gen_epoch(validation_dataset, batch_size=batch_size):
      feed_dict = self.build_feed_dictionary(batch_current_covariates=batch_current_covariates,
                                             batch_previous_treatments=batch_previous_treatments,
                                             batch_current_treatments=batch_current_treatments,
                                             batch_init_state=batch_init_state,
  ...
  def load_network(self, tf_session, model_dir, checkpoint_name):
    ...
    """
    load_path = \
      os.path.join(model_dir, "{0}.ckpt".format(checkpoint_name))
