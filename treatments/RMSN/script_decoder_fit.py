...
def process_seq_data(data_map, states, projection_horizon,
  ...
  seq2seq_outputs = np.zeros(
    (num_seq2seq_rows, projection_horizon, outputs.shape[-1]))  # we reconstruct raw outputs later
...
def decoder_fit(hyperparams_decoder_iptw, training_dataset, validation_dataset, projection_horizon, task, MODEL_ROOT):
  ...
  encoder_specifications = {
    'rnn_propensity_weighted': treatments.RMSN.configs.load_optimal_parameters('rnn_propensity_weighted',
                                                                               expt_name, MODEL_ROOT)
  }

  decoder_specifications = {
    'rnn_propensity_weighted': (hyperparams_decoder_iptw['dropout_rate'], hyperparams_decoder_iptw['memory_multiplier'],
                                hyperparams_decoder_iptw['num_epochs'], hyperparams_decoder_iptw['batch_size'],
                                hyperparams_decoder_iptw['learning_rate'], hyperparams_decoder_iptw['max_norm']),
  }
  ...
  for net_name in net_names:
    ...
    train_preds, _, _, train_states = test(training_processed, tf_config,
    ...
    valid_preds, _, _, valid_states = test(validation_processed,
                                           tf_config,
                                           net_name, expt_name, dropout_rate, num_features, num_outputs,
                                           memory_multiplier, num_epochs, minibatch_size, learning_rate, max_norm,
                                           hidden_activation, output_activation, model_folder,
    ...
    test_preds, _, _, test_states = test(test_processed, tf_config,
                                         net_name, expt_name, dropout_rate, num_features, num_outputs,
                                         memory_multiplier, num_epochs, minibatch_size, learning_rate, max_norm,
                                         hidden_activation, output_activation, model_folder,
                                         b_use_state_initialisation=False, b_dump_all_states=True)
    ...
    training_processed = process_seq_data(training_processed, train_states, projection_horizon=projection_horizon,
                                          num_features_to_include=num_outputs)
    validation_processed = process_seq_data(validation_processed, valid_states, projection_horizon=projection_horizon,
                                            num_features_to_include=num_outputs)
    test_processed = process_seq_data(test_processed, test_states, projection_horizon=projection_horizon,
                                      num_features_to_include=num_outputs)
    ...
    while True:
      ...
      if net_name not in decoder_specifications:
        ...
        adapter_multiplier = np.random.choice(
          [0.25, 0.5, 1, 1.5, 2.0]) if not b_single_layer and b_apply_memory_adapter else 0
        num_epochs = 100
      ...
      hyperparam_opt = train(seq_net_name, expt_name,
                             training_processed, validation_processed,
