...
class RMSN_Model(BaseEstimator, PredictorMixin):
  def __init__(self, hyperparams_encoder_iptw=None, hyperparams_decoder_iptw=None, model_dir=None, model_name=None,
               task=None, static_mode=None, time_mode=None):
    """
  ...
  def data_preprocess_counterfactuals(self, dataset, patient_id, timestep, treatment_options, fold, split,
    ...
    treatments_encoder = np.concatenate(
      [treatments_encoder, np.zeros(shape=(treat.shape[0], max_sequence_length - timestep,
                                           treat.shape[-1]))], axis=1)
  ...
  def fit(self, dataset, projection_horizon=None, fold=0, train_split='train', val_split='val'):
    ...
    # Train the model
    rnn_fit(self.hyperparams_encoder_iptw, dataset_train, dataset_val, networks_to_train='propensity_networks',
            task=self.task, MODEL_ROOT=self.MODEL_ROOT)
    propensity_generation(dataset_train, dataset_val, MODEL_ROOT=self.MODEL_ROOT)
    rnn_fit(self.hyperparams_encoder_iptw, dataset_train, dataset_val, networks_to_train='encoder', task=self.task,
            MODEL_ROOT=self.MODEL_ROOT)

    if (projection_horizon is not None):
      decoder_fit(self.hyperparams_decoder_iptw, dataset_train, dataset_val, projection_horizon=projection_horizon,
                  task=self.task,
                  MODEL_ROOT=self.MODEL_ROOT)
  ...
  def predict_counterfactual_trajectories(self, dataset, patient_id, timestep, treatment_options, fold=0,
    ...
    history, encoder_output, dataset_decoder = self.data_preprocess_counterfactuals(dataset=dataset,
