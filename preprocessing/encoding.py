...
class MinMaxNormalizer(BaseEstimator, DataPreprocessorMixin):
  ...
  def fit(self, dataset):
    ...
    if x is not None:
      ...
      temporal_norm_parameters = get_normalization_param(x[temporal_col_names], normalizer_type=self._normalizer_type)
    # For static data
    ...
    self.norm_parameters = {'normalizer': self._normalizer_type,
...
class ProblemMaker(BaseEstimator, DataPreprocessorMixin):
  ...
  def fit_transform(self, dataset):
    ...
    feature_name = {'temporal': temporal_features,
