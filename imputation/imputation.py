  ...
  def transform(self, dataset):
    ...
    if self.data_type == 'static':
      if dataset.static_feature is not None:
        ...
        dataset.static_feature = np.nan_to_num(dataset.static_feature, 0) * (1 - nan_mask) + self.vals * nan_mask
    elif self.data_type == 'temporal':
      if dataset.temporal_feature is not None:
        nan_mask = np.isnan(dataset.temporal_feature).astype(np.float)
        dataset.temporal_feature = np.nan_to_num(dataset.temporal_feature, 0) * (1 - nan_mask) + self.vals * nan_mask
...
class Imputation (BaseEstimator, DataPreprocessorMixin):
  ...
  def __init__(self, imputation_model_name, data_type):
    # Only allow for certain options
    assert imputation_model_name in ['mean', 'median','mice', 'missforest', 'knn','gain',
                                     'linear', 'quadratic', 'cubic', 'spline','mrnn','tgain']
    assert data_type in ['temporal', 'static']
    ...
    if self.data_type == 'static':
      if imputation_model_name in ['mean','median']:
        ...
        self.imputation_model = StandardImputation(imputation_model_name = imputation_model_name, data_type = 'static')
    ...
    # (2) Temporal imputation: 8 options
      ...
      if imputation_model_name in ['mean','median']:
        self.imputation_model = BasicImputation(imputation_model_name = imputation_model_name, data_type = 'temporal')
      elif imputation_model_name in ['linear', 'quadratic', 'cubic', 'spline']:
        self.imputation_model = Interpolation(interpolation_model_name = imputation_model_name, data_type = 'temporal')    
